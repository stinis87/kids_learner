...
| `red_light_green_light` | Play Red Light, Green Light: hide its eyes and wait a randomized moment, occasionally faking out with a lightning-fast one (`phase=green_light`), then snap back up, watch for movement, and turn to look at whoever gets caught (`phase=red_light`). | Core install only. Requires the camera (disable with `--no-camera`). Uses MediaPipe pose detection to tell moving people apart from background noise; downloads its pose landmarker model to the local cache on first use. |
