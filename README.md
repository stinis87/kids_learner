...
| `red_light_green_light` | Play Red Light, Green Light: turn away (`phase=green_light`) or turn back fast and check for movement (`phase=red_light`). | Core install only. Requires the camera (disable with `--no-camera`). |
