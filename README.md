...
| `red_light_green_light` | Play one full round of Red Light, Green Light per call: hides its eyes and waits a randomized moment, occasionally faking out with a lightning-fast one, then snaps back up, watches for movement, and turns to look at whoever gets caught. | Core install only. Requires the camera (disable with `--no-camera`). Detects movement by diffing consecutive camera frames and turns toward wherever the change is concentrated. |
