...
The token is a single Ring account credential shared by the whole app (not per-instance data), stored by default under your user data directory (`~/.local/share/reachy_mini_conversation_app/` on Linux/macOS). Override the location with `RING_TOKEN_CACHE_PATH`. Re-run `--ring-login` if the cached token expires. The Ring watcher's push notification subscription (see below) persists its own FCM registration alongside it in the same directory, so it doesn't need to re-register on every restart.
...
**Proactively reacting to motion/doorbell events**: beyond answering when asked, Reachy can also notice events on its own. This is enabled by default for every profile whenever Ring is configured — a profile can tune the cooldown, or opt out entirely with `enabled=false`, via an optional `ring_watcher.txt` file in its profile directory (see `profiles/default/ring_watcher.txt`). When enabled, Reachy subscribes to Ring's push notification stream — the same Firebase Cloud Messaging mechanism the Ring app itself uses, not polling — so it reacts in near real time to a genuinely new "motion"/"ding" event with a physical `play_emotion` reaction ("excited" for a doorbell ring, "attentive" for motion) plus fetching a fresh snapshot and prompting itself to look and comment. This is limited to at most once per device every `device_cooldown_seconds`, to avoid nagging about repeated motion (e.g. wind, passing cars) — a doorbell ring is never held back by this cooldown, since it's a deliberate, distinct action. Like `check_ring_camera`, the reaction snapshot is only ever sent to the realtime backend for analysis and never written to disk.
